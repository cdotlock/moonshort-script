...
路由声明块，**必须位于 `@episode` 块的尾部**。引擎按声明顺序从上到下判定 `@gate`，第一个命中的生效；全不命中则走 `@default`。
...
@gate main/bad/001:01 {
  type: choice
  trigger: A fail        // option A 且检定失败
}
```

`trigger` 格式：`<option_id> <check_result>`
...
@gate main/route/001:01 {
  type: influence
  condition: "玩家多次展现对弱者的同情心"
}
```

运行时，引擎将玩家积累的所有 `@butterfly` 记录喂给 LLM，判断是否满足 `condition`。

#### `@default <branch_key>`

兜底路线，所有 `@gate` 都不命中时走这里。
...
**`@<char> show <pose_expr> at <pos>`** — 角色入场
...
- `char`：角色 ID（小写）
- `pose_expr`：姿态_表情，对应素材语义名 `{char}_{pose_expr}`
...
**`@<char> expr <pose_expr> [transition]`** — 换表情/姿态

```
@mauricio expr arms_crossed_angry
...
- `@butterfly <description>`：引擎累积，供 `@gate type: influence` 的 LLM 判定
...
1. **解析**：将自定义语法解析为 AST
2. **验证**：语法正确性、引用完整性（角色是否 show 过才能 expr、label 是否存在等）
...
```json
{
      ...
      "options": [
        {
          ...
          "on_success": [
            {"type": "char_expr", "character": "easton", "pose_expr": "relieved", "url": "https://oss.mobai.com/..."},
          ...
          "on_fail": [
            {"type": "char_expr", "character": "easton", "pose_expr": "hurt", "url": "https://oss.mobai.com/..."},
        ...
        },
        {
          ...
          "steps": [
            {"type": "char_show", "character": "mark", "pose_expr": "grin_mischief", "url": "https://oss.mobai.com/...", "position": "right"},
  ...
  "gates": {
    "rules": [
      {
        "target": "main/bad/001:01",
        "type": "choice",
        "trigger": {"option_id": "A", "result": "fail"}
      },
      {
        "target": "main/route/001:01",
        "type": "influence",
        "condition": "玩家展现过对Easton的持续接纳"
      }
    ],
...
- **回归主线**：`@default main:02`
- **不回归**：`@default remix/abc123:02`（指向自己的下一集）
- **条件回归**：用 `@gate` 定义
...
| `@gates { }` | 路由声明（集尾部） |
| `@gate <branch_key> { type / trigger / condition }` | 跳转规则 |
| `@default <branch_key>` | 兜底路线 |
| `@label <name>` | 跳转锚点 |
| `@goto <name>` | 跳转（高级，慎用） |
| `@<char> show <pose_expr> at <pos>` | 角色入场 |
| `@<char> hide [transition]` | 角色退场 |
| `@<char> expr <pose_expr> [transition]` | 换表情 |
| `@<char> move to <pos>` | 角色移位 |
| `@<char> bubble <type>` | 气泡动画 |
