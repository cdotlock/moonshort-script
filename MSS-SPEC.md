...
- Achievement 是 inline step，step 本身携带完整元数据：`{"type":"achievement","achievement_id":"...","name":"...","rarity":"...","description":"..."}`。语义 id 用 `achievement_id` 字段（与所有 step 通用的 cursor `id` 字段区分，参见 §6.4 Step ID）。
...
`steps` 数组里**每个** step 都带一个必填的 `id: string` 字段，作为后端 player cursor 的稳定锚点。这是**冻结契约**——一旦 episode JSON 发到带持久 player session 的后端，编译器对 id 的算法就不能再变（除非配套数据迁移）。

注：上面 §6.3 的 JSON 示例为了突出各 step 自身的字段省略了 `id`，但实际编译器输出每个 step 都带它。完整带 id 的样例可参见 `testdata/feature_parade/ep01_output.json` 等 golden 文件。

#### 字段形态

格式 `<seq>_<tag>`，两段由下划线分隔：
...
- `phone_show.messages`（独立容器——内部的 text_message step 自己重置）

**并发组不是容器**：MSS 的 `&` 跟随节点会和 `@` 领导节点合并成一个 JSON 数组（见 §6.3），但组内成员在父容器的 seq 计数器里**继续递增**——不是嵌套重启 0001。例如顶层第 5 步是 `0005_dlg`，紧接的并发跟随节点是 `0006_char`，**不**是 `0001_char`。这与 runtime walker 的视图一致：并发组是平铺的兄弟槽位，不是嵌套边界。
...
由于容器作用域，`0001_dlg` 在同一份 JSON 里会出现**多次**——这是设计意图。后端 player cursor 用**完整路径**（带容器跳转段如 `"options"`、`"A"`、`"steps"`、`"then"`、`"else"`、`"messages"`）来定位，全路径才唯一。例：
...
- **不可改 tag**（重命名 `dlg` 为 `dialog`、把 `char` 拆细等）
- **不可改 seq 宽度**（4 位改成其他位数）
- **不可改容器规则**（增减容器、改并发组归属）
- **不可改起始值**（1-based → 0-based）

任何上述变更都会让所有持久化的 cursor 指错位置。如果实在要改，必须配套写一次性 cursor migration。详尽的 backend 集成指引见 `docs/ENGINE-INTEGRATION.md`。
