# MoonShort Script (MSS) 格式设计规范 v2.4
...
**已移除**。v2.3 及更早版本的 `@on success/fail` 和 `@on S/A B/C D` 块在 v2.4 彻底删除，以下两种标准 `@if` 语法取代：

- brave 选项内部：`@if (check.success) { } @else { }` —— `check.success` / `check.fail` 是新的 context-local Condition 类型，只在 brave option 体内合法
- minigame 内部：`@if (rating.<grade>) { } @else @if (rating.<grade>) { } ...` —— `rating.<grade>` 也是 context-local，只在 `@minigame` 体内合法

两个伪标识符在非法作用域内运行时恒为 false（**不**是 parse error）——作用域错放就是剧情 bug，不是语法问题。

遇到 `@on` 开头的行 parser 会给出明确错误和迁移提示；`mss fix` 也能识别并标记需要改写。
...
@achievement HIGH_HEEL_WARRIOR                   // 触发已声明的成就（见下文 @achievement 节）
```
...
事件信号，`kind` 必写。当前只实现 `mark` 一种 kind；kind 词元在源语法和 AST 中保留，以便未来扩展新 kind 时不破坏语法：

| kind | 用途 | 是否可被 `@if` 查询 |
|------|------|-------------------|
| `mark` | **持久布尔标记**。引擎永久存储，在 `@if (NAME)` 条件中作为布尔值使用。**只用于关键剧情点**——触发隐藏剧情、成就解锁守卫。 | 是（作为 `FlagCondition`） |

成就的触发**不**属于 signal——使用下面的 `@achievement <id>` 专用指令。
...
- **跨集 arc 成就**：两个或多个 mark 组合作为 `@if` 条件，守卫 `@achievement <id>` 触发。
...
// Achievement declaration — metadata only, no trigger logic
...
// Trigger lives in the body wherever the author wants it to fire:
@if (HIGH_HEEL_EP05) {
  @achievement HIGH_HEEL_WARRIOR
}
```
...
#### `@achievement <id>` — 声明与触发共用同一指令

`@achievement` 是一条语法，两种形态，parser 按是否跟 `{` 自动区分：

- `@achievement <id> { ... }` —— **声明块**。挂在 episode 顶层（可与 `@gate`/`@ending` 并存），只携带元数据。声明本身**不解锁**成就。
- `@achievement <id>` —— **触发指令**。作为 body step 就地发出成就事件。触发条件由外层 `@if` 控制（或裸触发，若无需条件）。

字段对齐 `cdotlock/story-achievement-generator` skill 的输出 schema——包含 name / rarity / description 三个必填字段（v2.4 移除 `when`）：
...
1. 剧本用 `@achievement <id> { ... }` 声明成就元数据（可在任何 episode）
2. 剧本用 `@signal mark NAME` 在恰当的剧情节点打下标记
3. 剧本在某个剧情节点用 `@achievement <id>` 触发——通常包裹在 `@if (mark1 && mark2 && ...) { @achievement <id> }` 里以确保条件成立
4. 引擎收到触发事件后走成就系统（UI 弹窗、成就页、数据上报）

**为什么去掉 `when`**：`when` 本质就是"引擎监听 marks 变化自动求值"的糖——剧作者失去了触发时机的控制权（想在玩家刚打完第二个 mark 的下一句对白触发 vs 在下一集开头触发，`when` 做不到）。改用 `@if + @achievement <id>` 后，引擎实现更简单（不必维护成就 watcher），剧作者也有明确的触发点控制。

代价：剧作者必须自己把触发点放到对的地方。最佳实践是**放在最后一个依赖 mark 的打点之后**——那时所有前置 mark 已经就位，`@if` 一次性判定即可。
...
**ID 约束**：
- 每个 episode 内 `@achievement` 声明 ID 不可重复
- 触发 ID 必须指向一个已声明的成就——runtime 注册表必须先见过声明。跨集引用由上层剧本管线（Dramatizer）保证唯一

**与成就生成 skill 的关系**：`@achievement { ... }` 声明块的字段结构镜像了 `cdotlock/story-achievement-generator` 生成的 JSON。成就生成器产出成就表后，剧作者（或 LLM 管线）将其落为 MSS 的声明块，加上对应的 `@signal mark` 打点与 `@if ... { @achievement <id> }` 触发即可。
...
- **brave / safe 选项的内容统一在 `steps` 字段**——brave 选项内部的 check 分支通过 `@if (check.success)` 的嵌套 if step 表达；safe 选项是直接的步骤列表。v2.3 及更早的 `on_success`/`on_fail` 字段已移除
- **minigame 的评级分支统一在 `steps` 字段**——每个 rating 分支通过 `@if (rating.<grade>)` 的嵌套 if step 表达。v2.3 及更早的 `on_results` 字段已移除
- CG 块输出 `duration`（`low`/`medium`/`high`）和 `content`（英文叙事串）字段；body 步骤在 `steps` 字段下
- Achievement 触发是独立的 step 类型：`{"type":"achievement","id":"..."}`。signal step 只输出 `mark` kind：`{"type":"signal","kind":"mark","event":"..."}`
...
| `@signal <kind> <event>` | 事件信号。目前只认 kind=`mark`（持久布尔标记）；kind 词元保留以便未来扩展 |
| `@achievement <id> { name / rarity / description }` | 成就声明（字段对齐 story-achievement-generator skill；v2.4 移除 `when`） |
| `@achievement <id>` | 成就触发（裸指令；通常包裹在 `@if (...) { @achievement <id> }` 里做条件触发） |
