...
- trick 集是**调色盘**，覆盖情绪寄存器：动作（tap / shake / swing）、克制（hold）、紧绷躲藏（hold-still）、方向手势（swipe）。
...
- **未入选及原因**（防止以后反复加）：
  ...
  - drag / twist / scratch / cover / flip / raise / hum / lean-in / tilt-head 等候选——考虑过，未入选，保持 6 个。
...
| `@trick <type> "<prompt>"` | 强制 trick（leaf；引擎原生；type ∈ tap / hold / swipe / shake / swing / hold-still） |
| `@choice { }` | 选择块 |
