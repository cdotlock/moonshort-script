...
条件格式：`<option_id>.<check_result>`（点号分隔）
...
两种写法均合法：`@if (influence "description")` 和 `@if ("description")`（裸字符串）。解析器会将裸字符串自动识别为 influence 条件。运行时，引擎将玩家积累的所有 `@butterfly` 记录喂给 LLM，判断是否满足条件。
...
@signal EP01_COMPLETE      // 向引擎抛出事件（同时作为持久布尔标记）
@butterfly "正面接受了Easton的靠近"  // 蝴蝶效应记录
```

> **引擎管理的数值**（如 XP、SAN/HP 等）由引擎内部维护，脚本**不能**修改它们。脚本只能在 `@if` 条件中引用这些数值（如 `@if (san <= 20) { }`），具体名称由引擎定义。

- `@signal <event>`：事件名可以是裸标识符或双引号字符串（`@signal EP01_COMPLETE` 和 `@signal "EP01_COMPLETE"` 均合法）。双重角色——
  1. **引擎通知**：向引擎抛出命名事件，用于成就、UI 提示、数据分析等
  2. **持久布尔标记**：引擎永久存储每个 signal。在 `@if` 条件中，signal 名称作为布尔值——被触发过为 TRUE，否则为 FALSE。这是触发隐藏剧情线的机制
  - 示例：`@signal MINIGAME_PERFECT_EP01` → 之后 `@if (MINIGAME_PERFECT_EP01) { NARRATOR: You remember that perfect run. }`
...
**条件类型（5 种，body `@if` 和 gate `@if` 均可使用）：**

| 类型 | 语法 | 说明 |
|------|------|------|
| choice | `<ID>.<result>` | 选项检定结果，如 `A.fail`、`B.success`、`C.any` |
| influence | `influence "desc"` 或 `"desc"` | LLM 蝴蝶效应判定（两种写法均合法） |
| flag/signal | `SIGNAL_NAME` | 布尔值，该 signal 是否被触发过 |
| comparison | `affection.<char> <op> <N>` 或 `<name> <op> <N>` | 数值比较（好感度或引擎管理的数值） |
| compound | `expr && expr` / `expr \|\| expr` | 复合条件 |
...
- `gate` 采用嵌套 if/else 链而非平坦数组，条件为结构化对象（含 `type` 字段）
...
| `@signal <event>` | 抛出事件（同时作为持久布尔标记，可在 `@if` 中检查） |
