...
@signal achievement HIGH_HEEL_WARRIOR            // 解锁声明过的成就
```
...
| kind | 用途 | 是否可被 `@if` 查询 |
|------|------|-------------------|
| `mark` | **持久布尔标记**。引擎永久存储，在 `@if (NAME)` 条件或 `@achievement` 的 `when` 中作为布尔值使用。**只用于关键剧情点**——触发隐藏剧情、成就触发条件。 | 是（作为 `FlagCondition`） |
...
1. 先想清楚后面哪里有条件查询要用它——某集的 `@if (X)` 隐藏剧情？某个 `@achievement` 的 `when: (X && Y)`？
...
- **跨集 arc 成就**：两个或多个 mark 组合触发一个 `@achievement` 的 `when` 条件。
...
// Achievement declaration — read via `when`
