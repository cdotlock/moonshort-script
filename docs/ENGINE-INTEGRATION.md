...
| Unlocked achievements | `{type:"achievement", id, name, rarity, description}` | — 纯通知，不反向查询 |
...
function handle_achievement(step):
    if unlock_store.contains(step.id):
        return  # 已解锁，不重复
    unlock_store.insert(step.id)
    ui.show_achievement_popup(step.name, step.rarity, step.description)
    analytics.report_achievement_unlock(step.id, step.rarity)
```
