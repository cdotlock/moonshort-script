...
| `achievements` | array | 该集声明的成就表（空时为 `[]`）。每项：`{id, name, rarity, description}`（v2.4 已移除 `when` 字段）；详见 §8 |
...
| `achievement` | 成就触发事件（按 id 引用已声明的成就） |
| `butterfly` | 蝴蝶效应记录 |
| `label` | 跳转锚点（引擎内部用） |
| `goto` | 跳转（引擎内部用） |
...
brave 和 safe 选项内容统一在 `steps` 字段下——v2.4 移除了 v2.3 及更早的 `on_success`/`on_fail` 字段。brave 的成功/失败分支通过嵌套的 `@if (check.success) { } @else { }` step 表达（condition type 是新加的 `check`）。
...
v2.4 起 body 步骤统一在 `steps` 字段下——评级分支通过嵌套的 `@if (rating.<grade>) { }` step 表达（condition type 是新加的 `rating`）。v2.3 及更早的 `on_results` 字段已移除。
...
v2.4 起只支持一种 kind（`mark`），但 `kind` 字段仍然保留在 JSON 输出中——方便未来扩展新 kind 时不破坏消费者。成就触发是独立的 `achievement` step 类型，不再走 signal。
...
```json
{ "type": "achievement", "id": "FIRST_KISS" }
```

MSS 源语法是裸 `@achievement <id>`（无 `{`）。通常写在 `@if (...)` 守卫里，当条件满足时就地触发：
...
引擎收到该 step 后，查找 episode JSON 顶层 `achievements` 数组中 `id == "HIGH_HEEL_DOUBLE_KILL"` 的条目，走成就系统：弹窗、解锁、数据上报。触发事件**不**影响 flag 存储——`@if (HIGH_HEEL_DOUBLE_KILL)` 查不到。

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | string | 是 | 成就 ID，必须指向已声明的 `@achievement { ... }` 条目 |
...
| `achievement` | `id: string` | 成就触发（按 id 引用已声明成就） |
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `id` | string | 是 | 成就唯一标识符。可被内联 `@achievement <id>` step 引用 |
| `name` | string | 是 | 显示名称（英文） |
| `rarity` | string | 是 | `"uncommon"` / `"rare"` / `"epic"` / `"legendary"`（**无 `common`**） |
| `description` | string | 是 | DM 口吻 flavor 文本（英文） |

v2.4 起声明块只携带元数据，**不包含 `when` 触发条件**——触发时机由脚本里的 `@achievement <id>` 内联 step 决定（通常包裹在 `@if` 里）。

### 8.2 触发方式

脚本里用 `@achievement <id>`（无 `{`）生成一个 `{"type":"achievement","id":"..."}` step，引擎收到后：

1. 在当前 episode（或全局注册表）的 `achievements` 数组里查找 `id` 匹配的条目
2. 走成就系统：UI 弹窗、解锁状态持久化、数据上报
3. 触发**不**影响 flag 存储——`@if (HIGH_HEEL_DOUBLE_KILL)` 永远查不到
...
跨集 arc 触发——把守卫放在最后一个依赖 mark 被写入之后：
```
// Episode 24
...
**约束**：触发的 `id` 必须指向已声明的成就；未声明就触发视为错误（引擎应忽略或记录诊断，不崩）。
